...
public class Iota {
    ...
    private void deleteViewModel(HashesViewModel model,String logName) throws Exception {
        int counter = 0;

        while (model != null) {
            if (++counter % 10000 == 0) {
                log.info("Clearing cache: {} "+logName, counter);
            }
            HashesViewModel NextAdd =  model.next(tangle);
            model.delete(tangle);
            model = NextAdd;
        }
