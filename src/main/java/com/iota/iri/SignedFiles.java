...
public class SignedFiles {
    ...
    public static boolean isFileSignatureValid(String filename, String signatureFilename, String publicKey, int depth, int index) {
        ...
        final int[] signature;
        try {
            signature = digestFile(filename, SpongeFactory.create(SpongeFactory.Mode.KERL));
        } catch (IOException | UncheckedIOException e) {
            log.error("Error digesting file: " + filename, e);
            return false;
        }
        ...
        try {
            ...
            log.error("Error validating signature file: " + signatureFilename, e);
            return false;
        }
    }
    ...
    private static int[] digestFile(String filename, Sponge curl) throws IOException, UncheckedIOException {
        try (InputStream inputStream = SignedFiles.class.getResourceAsStream(filename);
            ...
            reader.lines().forEach(line -> {
                Converter.trits(Converter.asciiToTrytes(line), buffer, 0);
