...
/**
 * Controller class for {@link Transaction} sets. A {@link TransactionViewModel}
 * stores a {@link HashesViewModel} for each component of the
 * {@link Transaction} within it.
 */
public class TransactionViewModel {
    ...
    /**
     * Total supply of IOTA available in the network. Used for ensuring a balanced
     * ledger state and bundle balances
     */
    public static final long SUPPLY = 2779530283277761L; // = (3^33 - 1) / 2

    /**
     * The predefined offset position and size (in trits) for the varying components
    ...
    /**
     * Stores the {@link HashesViewModel} for the {@link Transaction} components
     * here
     */
    ...
    /**
     * Populates the meta data of the {@link TransactionViewModel}. If the
     * controller {@link Hash} identifier is null, it will return with no response.
     * If the {@link Transaction} object has not been parsed, and the
     * {@link TransactionViewModel} type is <tt>FILLED_SLOT</tt>, the saved metadata
     * batch will be saved to the database.
     *
     * @param tangle               The tangle reference for the database.
     * @param transactionViewModel The {@link TransactionViewModel} whose Metadata
     *                             is to be filled.
    ...
    /**
     * Creates a new controller using a byte array that will be converted to a
     * {@link Hash} identifier. The new {@link TransactionViewModel} is created for
     * the {@link Transaction} object referenced by this {@link Hash} identifier,
     * provided the {@link Transaction} exists in the database.
     *
     * @param tangle The tangle reference for the database
     * @param hash   The source that the {@link Hash} identifier will be created
     *               from, and the {@link Transaction} object will be fetched from
     *               the database from
     * @return The {@link TransactionViewModel} with its Metadata filled in.
     * @throws Exception Thrown if the database fails to find the
    ...
    /**
     * Creates a new controller for a {@link Transaction} set referenced by a given
     * {@link Hash} identifier. The controller will be created and its metadata
     * filled provided the {@link Transaction} object exists in the database.
     *
     * @param tangle The tangle reference for the database
     * @param hash   The {@link Hash} identifier to search with
     * @return The {@link TransactionViewModel} with its Metadata filled in.
     * @throws Exception Thrown if there is an error loading the {@link Transaction}
    ...
    /**
     * Constructor for a {@link Transaction} set controller interface. This
     * controller is used to interact with and manipulate a provided
     * {@link Transaction} set.
     *
     * @param transaction {@link Transaction} set that the
     *                    {@link TransactionViewModel} will be created for
    ...
    /**
     * Constructor for a {@link Transaction} set controller interface. A new
     * {@link Transaction} set is created from the provided trit array, if the array
     * is of the correct size. If it is not the correct size, a new byte array of
     * the correct size is created, the trit array is copied into it, and the bytes
     * are stored in the new {@link Transaction} set. This {@link Transaction} set
     * is then indexed by the provided {@link Hash} identifier.
     *
     * @param trits The input trits that the {@link Transaction} and
     *              {@link TransactionViewModel} will be created from.
     * @param hash  The {@link TransactionHash} identifier of the
     *              {@link Transaction} set
     */
     ...
     * {@link com.iota.iri.storage.rocksDB.RocksDBPersistenceProvider} to determine
     * if the {@link Transaction} object might exist in the database. If it
     * definitively does not exist, it will return False.
     *
     * @param tangle The tangle reference for the database
     * @param hash   The {@link Hash} identifier of the object you are looking for
     * @return True if the key might exist in the database, False if it definitively
     *         does not
    ...
    /**
     * Determines whether the {@link Transaction} object exists in the database or
     * not.
     *
     * @param tangle The tangle reference for the database.
     ...
     * @throws Exception Thrown if there is an error determining if the transaction
     *                   exists or not
     */
     ...
     *
     * @param tangle The tangle reference for the database.
     * @return The integer count of total {@link Transaction} objects in the
     *         database
    ...
    /**
     * Converts the given byte array to the a new trit array of length
     * {@value TRINARY_SIZE}.
     * 
    ...
    /**
     * Fetches the first persistable {@link Transaction} object from the database
     * and generates a new {@link TransactionViewModel} from it. If no objects exist
    ...
    /**
     * This method updates the metadata contained in the {@link Transaction} object,
     * and updates the object in the database. First, all the most recent
     * {@link Hash} identifiers are fetched to make sure the object's metadata is up
     * to date. Then it checks if the current {@link TransactionHash} is null. If it
     * is, then the method immediately returns false, and if not, it attempts to
     * update the {@link Transaction} object and the referencing {@link Hash}
     * identifier in the database.
     *
     * @param tangle          The tangle reference for the database
     * @param initialSnapshot snapshot that acts as genesis
     * @param item            The string identifying the purpose of the update
     * @throws Exception Thrown if any of the metadata fails to fetch, or if the
    ...
    /**
     * Retrieves the {@link TransactionViewModel} for the branch {@link Transaction}
     * object referenced by this {@link TransactionViewModel}. If the controller
     * doesn't already exist, a new one is created from the branch transaction
     * {@link Hash} identifier.
     *
     * @param tangle The tangle reference for the database.
     * @return The branch transaction {@link TransactionViewModel}
     * @throws Exception Thrown if no branch is found when creating the branch
     *                   {@link TransactionViewModel}
     */
    ...
    /**
     * Retrieves the {@link TransactionViewModel} for the trunk {@link Transaction}
     * object referenced by this {@link TransactionViewModel}. If the controller
     * doesn't already exist, a new one is created from the trunk transaction
     * {@link Hash} identifier.
     *
     * @param tangle The tangle reference for the database.
     * @return The trunk transaction {@link TransactionViewModel}
     * @throws Exception Thrown if no trunk is found when creating the trunk
     *                   {@link TransactionViewModel}
     */
    ...
    /**
     * @return The trits stored in the {@link TransactionViewModel} . If the trits
     *         aren't available in the controller, the trits are generated from the
     *         transaction object and stored in the controller {@link #trits()}
     *         variable.
     */
    ...
    /**
     * Stores the {@link Transaction} object to the tangle, including the metadata
     * and indexing based on {@link Bundle}, {@link Address}, {@link Tag},
     * {@link #trunk} and {@link #branch}.
     *
     * @return The list of {@link Hash} objects indexed by the
     *         {@link TransactionHash} identifier. Returns False if there is a
     *         problem populating the list.
     */
    ...
    /**
     * Fetches a list of all {@link Transaction} component and {@link Hash}
     * identifier pairs from the stored metadata. The method then ensures that the
     * {@link Transaction#bytes} are present before adding the {@link Transaction}
     * and {@link Hash} identifier to the already compiled list of
     * {@link Transaction} components.
     *
     * @return A complete list of all {@link Transaction} component objects paired
     *         with their {@link Hash} identifiers
     * @throws Exception Thrown if the metadata fails to fetch, or if the bytes are
     *                   not retrieved correctly
     */
    ...
    /**
     * Fetches the next indexed persistable {@link Transaction} object from the
     * database and generates a new {@link TransactionViewModel} from it. If no
     * objects exist in the database, it will return null.
     *
     * @param tangle the tangle reference for the database.
     * @return The new {@link TransactionViewModel}.
    ...
    /**
     * This method fetches the saved batch of metadata and orders them into a list
     * of {@link Hash} objects and {@link Hash} identifier pairs. If the
     * {@link Hash} identifier of the {@link Transaction} is null, or the database
     * already contains the {@link Transaction}, then the method returns False.
     * Otherwise, the method tries to store the {@link Transaction} batch into the
     * database.
     *
     * @param tangle          The tangle reference for the database.
     * @param initialSnapshot snapshot that acts as genesis
     * @return True if the {@link Transaction} is stored, False if not.
     * @throws Exception Thrown if there is an error fetching the batch or storing
     *                   in the database.
     */
    ...
    public Transaction getTransaction() {
        ...
        // if the supplied array to the call != null the transaction bytes are copied
    ...
    /**
     * Gets the {@link ApproveeViewModel} of a {@link Transaction}. If the current
     * {@link ApproveeViewModel} is null, a new one is created using the transaction
     * {@link Hash} identifier.
     *
     * An {@link Approvee} is a transaction in the tangle that references, and
     * therefore approves, this transaction directly.
     *
     * @param tangle The tangle reference for the database
     * @return The {@link ApproveeViewModel}
     * @throws Exception Thrown if there is a failure to create a controller from
    ...
    /**
     * Gets the stored {@link Transaction#bytes}. If the {@link Transaction#bytes}
     * are null, a new byte array is created and stored from the {@link #trits}. If
     * the {@link #trits} are also null, then a null byte array is returned.
     *
     ...
     * @throws Exception If the address cannot be found in the database, an
     *                   exception is thrown.
     */
    ...
    /**
     * Gets the {@link Transaction#attachmentTimestamp}. The
     * <tt>Attachment Timestapm</tt> is used to show when a transaction has been
    ...
    /**
     * Gets the {@link Transaction#attachmentTimestampLowerBound}. The
     * <tt>Attachment Timestamp Lower Bound</tt> is the earliest timestamp a
    ...
    /**
     * Gets the {@link Transaction#attachmentTimestampUpperBound}. The
     * <tt>Attachment Timestamp Upper Bound</tt> is the maximum timestamp a
     * transaction can have.
     *
     ...
     * @param tangle          The tangle reference for the database
     * @param initialSnapshot snapshot that acts as genesis
     * @param validity        The state of validity that the {@link Transaction}
     *                        will be updated to
    ...
    /**
     * Creates an array copy of the signature message fragment of the
     * {@link Transaction} and returns it.
     * 
    ...
    /**
     * Creates a byte array of the size {@value NONCE_TRINARY_SIZE} and copies the
     * nonce section of the transaction trits into the array. This array is then
     * returned.
     *
    ...
    /**
     * Fetches the {@link Transaction#tag}, and converts the transaction trits for
     * the {@link Transaction#attachmentTimestamp}, the
     * {@link Transaction#attachmentTimestampLowerBound}, and the
     * {@link Transaction#attachmentTimestampUpperBound} to long values.The method
     * then sets these values to the {@link TransactionViewModel} metadata.
     */
    ...
    /**
     * Converts the {@link Transaction#value}, {@link Transaction#timestamp},
     * {@link Transaction#currentIndex} and {@link Transaction#lastIndex} from trits
     * to long values and assigns them to the {@link TransactionViewModel} metadata.
     * The method then determines if the {@link Transaction#bytes} are null or not.
     * If so the {@link Transaction#type} is set to {@link #PREFILLED_SLOT}, and if
     * not it is set to {@link #FILLED_SLOT}.
     */
    public void setMetadata() {
        ...
        // if (transaction.timestamp > 1262304000000L ) transaction.timestamp /= 1000L;
        // // if > 01.01.2010 in
        // milliseconds
    ...
    public static void updateSolidTransactions(Tangle tangle, Snapshot initialSnapshot,
            final LinkedHashSet<Hash> analyzedHashes) throws Exception {
    ...
    /**
     * Updates the {@link Transaction#solid} value of the referenced
     * {@link Transaction} object.
     *
     * Used by the {@link com.iota.iri.TransactionValidator} to quickly set the
     * solidity of a {@link Transaction} set.
     *
    ...
    /**
     * @return True if {@link Transaction#solid} is True (exists in the database),
     *         False if not
     */
     ...
     * @param tangle          The tangle reference for the database.
     * @param initialSnapshot snapshot that acts as genesis
     * @param index           The new index to be attached to the
     *                        {@link Transaction} object
     ...
     *
     * It gets automatically called by the
     * {@link com.iota.iri.service.milestone.LatestMilestoneTracker} and marks
     * transactions that represent a milestone accordingly. It first checks if the
     * {@link Transaction#milestone} flag has changed and if so, it issues a
     * database update.
     *
     * @param tangle          Tangle instance which acts as a database interface
     * @param initialSnapshot the snapshot representing the starting point of our
     *                        ledger
     * @param isMilestone     true if the transaction is a milestone and false
     *                        otherwise
     * @throws Exception if something goes wrong while saving the changes to the
     *                   database
     */
     ...
     *
     * The {@link Transaction#milestone} flag indicates if the {@link Transaction}
     * is a coordinator issued milestone. It allows us to differentiate the two
     * types of transactions (normal transactions / milestones) very fast and
     * efficiently without issuing further database queries or even full
     * verifications of the signature. If it is set to true one can for example use
     * the snapshotIndex() method to retrieve the corresponding
     ...
     * transaction.
     *
     * It can be used to directly append the transaction in error and debug
     * messages.
     *
