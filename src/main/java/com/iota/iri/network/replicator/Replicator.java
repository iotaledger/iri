 ...
 * <br>
 * A **Source** is basically a receive end point for TCP where all  the peers send
 * transactions to. Since there is only one global endpoint for all the peers, we need
 * only a single thread to manage all the incoming messages. 
 * <br>
 * A **Sink** is a peer where we sends packets to, therefore we need multiple worker
