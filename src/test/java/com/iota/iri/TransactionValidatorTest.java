...
public class TransactionValidatorTest {
  ...
  public void validateBytesWithNewCurl() throws Exception {
    ...
    Converter.copyTrits(0, trits, 0, trits.length);
    byte[] bytes = Converter.bytes(trits);
    TransactionValidator.validate(bytes, txValidator.getMinWeightMagnitude(), SpongeFactory.create(SpongeFactory.Mode.CURL));
  }
  ...
  public void addSolidTransactionWithoutErrors() {
    int[] trits = getRandomTransactionTrits();
    Converter.copyTrits(0, trits, 0, trits.length);
    txValidator.addSolidTransaction(Hash.calculate(SpongeFactory.Mode.CURL, trits));
  }
  ...
  private TransactionViewModel getTxWithBranchAndTrunk() throws Exception {
    ...
    tx = new TransactionViewModel(childTx, Hash.calculate(SpongeFactory.Mode.CURL, childTx));
  ...
  private TransactionViewModel getTxWithoutBranchAndTrunk() throws Exception {
    int[] trits = getRandomTransactionTrits();
    TransactionViewModel tx = new TransactionViewModel(trits, Hash.calculate(SpongeFactory.Mode.CURL, trits));
