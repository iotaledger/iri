...
public class LocalSnapshotManagerImplTest {
    ...
    public synchronized void takeLocalSnapshot() throws Exception {
        ...
        when(milestoneSolidifier.getLatestMilestoneIndex()).thenReturn(200);
        ...
        Thread t = new Thread(() -> lsManager.monitorThread(milestoneSolidifier));
    ...
    public void isInSyncTestScanIncomplete() {
        ...
        assertFalse("We should be out of sync when he havent finished initial scans", lsManager.isInSync(milestoneSolidifier));
    }
    ...
    public void isInSyncTestScanComplete() {
        ...
        // -5 & -1 -> not in sync
        assertFalse("Previous out of sync and not equal index should not be in sync", lsManager.isInSync(milestoneSolidifier));
        
        // -1 and 5 -> not in sync
        assertFalse("Previous out of sync and not equal index should not be in sync", lsManager.isInSync(milestoneSolidifier));
        
        // 10 and 10 -> in sync
        assertTrue("Equal index should be in sync", lsManager.isInSync(milestoneSolidifier));
        
        // 998 and 1002 -> in sync since sync gap = 5
        assertTrue("Index difference less than the buffer still should be in sync", lsManager.isInSync(milestoneSolidifier));
        
        // 999 and 2000 -> out of sync again, bigger gap than 5
        assertFalse("Index difference more than the buffer should be out of sync again ", lsManager.isInSync(milestoneSolidifier));
